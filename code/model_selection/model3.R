...
model_code <- nimbleCode({
  ...
  for (i in 1:nZones) {
    ...
    for (t in 2:nYears) {
      ...
      logit(gamma[i, t - 1]) <- colonization(CPUE[1:nZones, t], 
        beta0, beta1, 
...
out <- clusterEvalQ(cl, {
  ...
  colonization <- nimbleFunction(
                   ...
                   Kscale = double(0))
    {
      ...
      for (j in 1:n) {
        if (j != idx) {
          gamma <- gamma + beta1 * CPUE[j] * C[j] / Kscale 
        }
      }
      gamma <- gamma + beta0
      return(gamma)
...
# save samples
saveRDS(out_sub, "data/posterior_samples/model_selection/model3.rds")
