...
model_code <- nimbleCode({
  ...
  for (i in 1:nZones) {
    ...
    for (t in 2:nYears) {
      ...
      logit(epsilon[i, t - 1]) <- persistence(beta1, beta2, 
        C[i, i, t], Kscale)
    }
  }
...
out <- clusterEvalQ(cl, {
  ...
  persistence <- nimbleFunction(
    run = function(beta1 = double(0), beta2 = double(0), 
                   C = double(0), Kscale = double(0))
    {
      returnType(double(0))
      epsilon <- beta1 + C * beta2 / Kscale
      return(epsilon)
    }
  )
  ...
  mcmcConf_myModel <- configureMCMC(
    myModel,
    monitors = c("psi", "beta0", "beta1", "beta2", 
...
# save samples
saveRDS(out_sub, "data/posterior_samples/model_selection/model7.rds")
