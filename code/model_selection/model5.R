...
model_code <- nimbleCode({
  ...
  for (i in 1:nZones) {
    ...
    for (t in 2:nYears) {
      ...
      logit(gamma[i, t - 1]) <- colonization(beta0, beta1, 
        C[i, 1:nZones, t], i, Kscale)
      
      # Probability of persistence
      logit(epsilon[i, t - 1]) <- persistence(beta2, beta3, 
        C[i, i, t], Kscale)
    }
  }
...
out <- clusterEvalQ(cl, {
  ...
  colonization <- nimbleFunction(
    run = function(beta0 = double(0), beta1 = double(0), 
                   C = double(1), index = double(0),
                   Kscale = double(0))
    {
      returnType(double(0))
  ...
  persistence <- nimbleFunction(
    run = function(beta2 = double(0), beta3 = double(0), 
                   C = double(0), Kscale = double(0))
    {
      returnType(double(0))
      epsilon <- beta2 + C * beta3 / Kscale
      return(epsilon)
    }
  )
...
# save samples
saveRDS(out_sub, "data/posterior_samples/model_selection/model5.rds")
