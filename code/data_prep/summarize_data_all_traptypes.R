...
#
# PATHS: relative to the repository root (EGC_Occupancy_BM). Set the working
# directory to the repo root before running. Inputs live in Data/Raw; outputs
# are written to Data/Processed and Data/.
#
...
# ---------------------------------------------------------------------------
# Locate the repo root automatically so the script runs regardless of the
# current working directory. This script lives in Code/, so the repo root is
# its parent folder. Works when sourced, run via Rscript, or run from the
# RStudio editor. Falls back to the current working directory.
# ---------------------------------------------------------------------------
get_script_dir <- function() {
  ...
  # source()'d: ofile in one of the call frames
  for (i in rev(seq_len(sys.nframe()))) {
    of <- sys.frame(i)$ofile
    if (!is.null(of)) return(dirname(normalizePath(of)))
  }
  # RStudio editor (running line-by-line or via the Source button)
...
if (!is.null(script_dir)) {
  setwd(normalizePath(file.path(script_dir, "..")))  # Code/ -> repo root
}

# sanity check: make sure we found the data
if (!dir.exists("Data/Raw")) {
  stop("Could not find 'Data/Raw' relative to the working directory (",
       getwd(), "). Set the working directory to the repo root manually, ",
       "e.g. setwd('C:/Users/makhl/Research Repos/EGC_Occupancy_BM').")
}
...
write.csv(all_sources_final,
          file.path("Data", "data_all_sources_all_traptypes.csv"),
          row.names = FALSE)
