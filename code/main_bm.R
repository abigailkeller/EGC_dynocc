# main_bm.R
#
# Same dynamic occupancy model as main.R, run against the extended data
# products from code/data_prep/BayesianDataSitesAdded.R: the 136 real
# (trap-level) sites plus 56 imputed sites from aggregate-only sources (WSG,
# Padilla Bay, Drayton Harbor) that 02_BayesianTrapData.R excludes. See that
# script's header comment for exactly how those 56 sites' presence/absence
# and trap-type data were imputed from season-pooled catch/effort.
#
# The only changes from main.R are the four input files (the _xtraSites /
# site_zone_map_xtraSites versions instead of the originals) and the output
# path (posterior_samples_bm.rds, so this doesn't overwrite the real run's
# results). The model code itself -- priors, colonization/persistence
# functions, observation model, MCMC configuration -- is untouched.
...
setwd("C:/Users/makhl/Research Repos/PNNL_Repos/EGC_dynocc")
################
# read in data #
################

cpue <- readRDS("data/model_data/cpue_xtraSites.rds")[, 1:6]
detections <- readRDS("data/model_data/PresenceArrayBinary_xtraSites.rds")[, 2:7, ]
type <- readRDS("data/model_data/TrapTypeArraysNew_xtraSites.rds")
zones <- read.csv("data/model_data/site_zone_map_xtraSites.csv")[, "zone_id"]
...
# NA for Campbell Slough (still the only unmapped site with the added sites in)
...
# save samples
saveRDS(out_sub, "data/posterior_samples/posterior_samples_bm.rds")
